...
decoder. MLX 0.32.2 is the current release and exposes no CPU
reduction-precision setting. The exact next step is to obtain or implement a
justified native kernel that reproduces the PyTorch RMSNorm arithmetic, then
rerun `uv run pytest tests/test_prefix.py tests/test_language.py -q`.
