...
- [x] Commit and push to `origin`: closing evidence checkpoint `8934840` is on `main`, with a clean worktree and the exact same remote SHA verified. Software handoff is in `docs/evidence/TRAINED_PARITY_REPAIR.md`; physical validation remains deferred until the operator returns home. This checklist closure is a documentation-only follow-up.
